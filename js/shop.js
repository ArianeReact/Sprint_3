...
//Number((cartItem.subTotal - cartItem.subTotal * cartItem.offer.percent/100).toFixed(2));
...
function cleanCart() {
    ...
    countProduct = 0;
    document.getElementById("count_product").innerHTML = countProduct;
    console.log("array cartList si cleanCart");
    console.table(cartList);
...
function applyPromotionsCart() {
    ...
    for(i = 0; i < cart.length; i++){
        ...
        if((cart[i].name == "Instant cupcake mixture") && (cart[i].quantity >= cart[i].offer.number)){
    ...
    console.log("array cart promotions");
    console.table(cart); 
...
function printCart() {
    ...
    for (let i = 0; i < cart.length; i++) {
        ...
        if(cart[i].subtotalWithDiscount){
            ...
            cart_list += "<td>" + cart[i].subtotal + "</td>";
            total_price += cart[i].subtotal;
        }
        cart_list += "</tr>";   
    }

    //Pintem per pantalla la variable llistaClients, ara amb tota l'array maquetada
